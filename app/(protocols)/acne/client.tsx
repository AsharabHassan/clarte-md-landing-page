...
'use client';
//
// Migrated from acne-protocol.html lines 2935-3313 (the trailing <script> block).
// The body is preserved VERBATIM aside from three additions for ai_session_id wiring,
// flagged with // NEW (Task 22) so a diff against the original file is trivial.
//
// @ts-nocheck applies because this is vanilla DOM JavaScript pulled in unchanged —
// it accesses elements like document.getElementById('fileInput').files[0] without
// HTMLInputElement casts. Typing each access would mutate ~50 sites for no behavioural
...
export default function AcneClient() {
  ...
  const PRODUCTS = {
    prep:   { name: 'Radiance Prep Cleanser',       price: 1799, list: 2000, actives: 'PHA 4% · Aloe',                         image: 'https://cdn.shopify.com/s/files/1/0782/5113/1112/files/Prep.png?v=1773743330' },
    rescue: { name: 'Clarifying Rescue Face Wash',  price: 1799, list: 2000, actives: 'Salicylic 2% · Zinc',                    image: 'https://cdn.shopify.com/s/files/1/0782/5113/1112/files/wash.png?v=1773743070' },
    vitc:   { name: 'Vitamin CE Ferrulic Serum',    price: 2250, list: 2950, actives: 'Vit C 15% · Vit E · Ferulic',            image: 'https://cdn.shopify.com/s/files/1/0782/5113/1112/files/3390b799-35fe-425b-bae9-41e4c8e41139.png?v=1773338016' },
    acne:   { name: 'Clarifying Acne Serum',        price: 2100, list: 3000, actives: 'Niacinamide 10% · Azelaic 10%',          image: 'https://cdn.shopify.com/s/files/1/0782/5113/1112/files/Generated_Image_March_13_2026_-_3_48AM.png?v=1773743197' },
    ha:     { name: 'Hyaluronic Acid Serum',        price: 2000, list: 2500, actives: 'HA · Panthenol',                         image: 'https://clartemd.com.pk/cdn/shop/files/d9a4c8e3-fcbb-4411-b5a3-fb59422d0040.png' },
    reti:   { name: 'Retinol Serum',                price: 2000, list: 2500, actives: 'Retinol 0.5%',                           image: 'https://cdn.shopify.com/s/files/1/0782/5113/1112/files/Gemini_Generated_Image_rwcfs4rwcfs4rwcf.png?v=1773881855' },
    light:  { name: 'Radiance Lightening Cream',    price: 2500, list: 4500, actives: 'Tranexamic 3% · Kojic · Arbutin',        image: 'https://cdn.shopify.com/s/files/1/0782/5113/1112/files/Generated_Image_March_13_2026_-_3_37AM.png?v=1773743441' },
    spf:    { name: 'Barrier Restore SPF 50+',      price: 1900, list: 2500, actives: 'SPF 50+ PA++++ · Centella',              image: 'https://cdn.shopify.com/s/files/1/0782/5113/1112/files/91edf02b-ef9c-4062-a6b9-f0975d941393.png?v=1773337514' },
  };
  const CROSS_SELLS = [
    { sku: 'prep',  why: 'Pair with the Rescue Wash for a gentler morning cleanse.' },
    { sku: 'vitc',  why: 'Accelerates the fading of dark post-acne pigmentation.' },
    { sku: 'reti',  why: 'For mature skin presenting both breakouts and fine lines.' },
  ...
  function renderCart() {
    ...
    if (items.length === 0) {
      sumEl.innerHTML = `<p style="color:rgba(255,255,255,0.55); font-family: 'Fraunces',serif; font-style: italic; font-size:14px; text-align:center; padding:18px 0;">Add the Clear Skin Protocol to begin.</p>`;
    } else {
  ...
  // The Camera button now opens a live getUserMedia preview instead of the
  // native file picker. The original markup is a <label for="cameraInput">
  // — strip the `for` so the label no longer auto-triggers the file input
  // (preventDefault on label clicks is inconsistent across browsers for
  // label→input activation). Then attach our async handler.
  ...
  btnCameraEl.addEventListener('click', async e => {
    ...
    try {
      ...
      // Cancelled or no permission — silently fall back to the file input
      // (which on mobile still uses capture="user" → native camera).
  ...
  // Paced for gpt-image-2 quality:medium — ~45–75s total. Each step ticks
  // every ~7s. The last step parks the bar at 96% so the bar fills the
  // moment the API response actually arrives, not before.
  const PROGRESS_STEPS = [
    { pct: 8,  msg: 'Reading texture, tone, and inflammation markers…' },
    { pct: 20, msg: 'Mapping breakout topography…' },
    { pct: 34, msg: 'Modelling niacinamide and azelaic response…' },
    { pct: 48, msg: 'Projecting twelve-week dermal renewal…' },
    { pct: 62, msg: 'Reconstructing post-acne hyperpigmentation…' },
  ...
  // Render the GPT-4o vision skin-map into a dedicated panel inside stageResult.
  // Created once on first call; subsequent calls update in place.
  function renderSkinMap(map) {
    ...
    if (!panel) {
      ...
      // Insert before any existing "result-actions" / CTA bar at the bottom of the result.
    ...
    panel.innerHTML = `
      ...
      <div class="skin-map-grid">
        ${map.primary_concerns?.length ? `
          <div class="skin-map-col">
            <div class="skin-map-label mono">Primary concerns</div>
            <ul class="skin-map-list">${pillsHTML(map.primary_concerns)}</ul>
          </div>` : ''}
        ${map.secondary_concerns?.length ? `
          <div class="skin-map-col">
            <div class="skin-map-label mono">Secondary</div>
            <ul class="skin-map-list muted">${pillsHTML(map.secondary_concerns)}</ul>
          </div>` : ''}
        ${map.recommended_actives?.length ? `
          <div class="skin-map-col">
            <div class="skin-map-label mono">Recommended actives</div>
            <ul class="skin-map-list cobalt">${pillsHTML(map.recommended_actives)}</ul>
          </div>` : ''}
      </div>
      ${map.warnings?.length ? `
  ...
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!bundleInCart && Object.keys(crossSellCart).length === 0) { alert('Your bundle is empty. Please add the Clear Skin Protocol to continue.'); return; }
