...
'use client';
//
// Migrated from Folder B's renewal-protocol.html lines 2538-2875.
...
export default function RenewalClient() {
  useEffect(() => {
  // ─────────────────── CONFIG ───────────────────
  const CONCERN = 'anti-ageing';
  const BUNDLE = {
    sku: 'renewal-protocol',
  ...
  const CROSS_SELLS = [
    { sku: 'rescue', why: 'For mixed adult acne and ageing concerns, swap in the Rescue Wash on alternate evenings.' },
    { sku: 'acne',   why: 'Hormonal adult breakouts respond to niacinamide alongside retinol.' },
  ...
  function renderCart() {
    ...
    if (items.length === 0) {
      sumEl.innerHTML = `<p style="color:rgba(255,255,255,0.55); font-family: 'Fraunces',serif; font-style: italic; font-size:14px; text-align:center; padding:18px 0;">Add the Renewal Protocol to begin.</p>`;
    } else {
  ...
  const PROGRESS_STEPS = [
    { pct: 8,  msg: 'Reading texture, fine lines, and tone markers…' },
    { pct: 20, msg: 'Mapping wrinkle topography…' },
    { pct: 34, msg: 'Modelling retinol and L-ascorbic response…' },
    { pct: 48, msg: 'Projecting twelve-week collagen renewal…' },
    { pct: 62, msg: 'Reconstructing softened lines and refined texture…' },
  ...
  async function startGeneration() {
    ...
    try {
      ...
      await openLeadGate({ surface: 'renewal', concern: 'renewal', ai_session_id: lastAiSessionId || undefined, skin_map: lastSkinMap || undefined });
  ...
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!bundleInCart && Object.keys(crossSellCart).length === 0) { alert('Your bundle is empty. Please add the Renewal Protocol to continue.'); return; }
    ...
    const freeShip = subtotal >= FREE_SHIP_MIN;
    const payload = {
      concern: CONCERN, page: 'renewal-protocol',
