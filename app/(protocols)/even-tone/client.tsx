...
'use client';
//
// Migrated from Folder B's even-tone-protocol.html lines 2529-2867 (the trailing <script> block).
// Same scaffold + ai_session_id wiring + window.addBundleToCart/toggleCrossSell exposure
...
export default function EvenToneClient() {
  useEffect(() => {
  // ─────────────────── CONFIG ───────────────────
  const CONCERN = 'pigmentation';
  const BUNDLE = {
    sku: 'even-tone-protocol',
  ...
  const CROSS_SELLS = [
    { sku: 'rescue', why: 'For mixed acne and pigmentation, swap in the Rescue Wash at night.' },
    { sku: 'acne',   why: 'If pigmentation is post-acne, treat both at once.' },
    { sku: 'ha',     why: 'Hydration support — actives work better on a healthy barrier.' },
  ...
  function renderCart() {
    ...
    if (items.length === 0) {
      sumEl.innerHTML = `<p style="color:rgba(255,255,255,0.55); font-family: 'Fraunces',serif; font-style: italic; font-size:14px; text-align:center; padding:18px 0;">Add the Even Tone Protocol to begin.</p>`;
    } else {
  ...
  async function startGeneration() {
    ...
    try {
      ...
      await openLeadGate({ surface: 'even-tone', concern: 'even-tone', ai_session_id: lastAiSessionId || undefined, skin_map: lastSkinMap || undefined });
  ...
  document.getElementById('orderForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!bundleInCart && Object.keys(crossSellCart).length === 0) { alert('Your bundle is empty. Please add the Even Tone Protocol to continue.'); return; }
    ...
    const freeShip = subtotal >= FREE_SHIP_MIN;
    const payload = {
      concern: CONCERN, page: 'even-tone-protocol',
